...
reflect_channel_messages = function reflect_channel_messages(msg_data) {
  ...
  $('.p-index-main__msg-list').empty();
  var prev_date = msg_data.messages[0].date;
  msg_data.messages.forEach(function (m, index) {
    // 最初のページ or 直前のメッセージの日付の翌日の場合、日付切替のボーダーを挿入
    if (index == 0 || m.date > prev_date) {
...
execute_message_addition = function execute_message_addition(chnl_id, content) {
  ...
  $.ajax({
    ...
    if (data.result) {
      ...
      if ($('.p-index-main__date-partition').length && data.message.date > $('.p-index-main__date-partition').last().text()) insert_date_partition(data.message.date);
      // メッセージを反映
...
//! 未使用
var ID = $('#user-settings').data('user_id');
...
execute_account_image_update = function execute_account_image_update(form_data) {
  $.ajax({
    ...
    if (data.result) {
      if (form_data.b64) {
        USER_IMAGE.html("<img src=\"".concat(form_data.b64, "\" alt=\"\" data-status=\"off\">"));
      } else {
        USER_IMAGE.html("<i class='fa-solid fa-user'></i>");
      }
...
execute_user_name_update = function execute_user_name_update() {
  console.log("execute_user_name_update");
  console.log(UPDATE_USER_NAME.val().trim());
  console.log(UPDATE_USER_NAME_PSW.val());
  $.ajax({
    data: {
      name: UPDATE_USER_NAME.val().trim(),
    ...
    console.log(data);
    if (data.result) {
      USER_INFO_NAME.text(UPDATE_USER_NAME.val().trim());
...
execute_user_email_update = function execute_user_email_update() {
  console.log("execute_user_email_update");
  $.ajax({
    data: {
      keyword: $('#server-search-bar').val()
    },
    type: 'POST',
    url: "/account/".concat(ID, "/update_email")
  }).done(function (data) {});
};
...
execute_user_psw_update = function execute_user_psw_update() {
  console.log("execute_user_psw_update");
  $.ajax({
    data: {
      keyword: $('#server-search-bar').val()
    },
    type: 'POST',
    url: "/account/".concat(ID, "/update_account_password")
  }).done(function (data) {});
};
...
execute_leave_account = function execute_leave_account() {
  console.log("execute_leave_account");
  $.ajax({
    data: {
      keyword: $('#server-search-bar').val()
    },
    type: 'POST',
    url: "/account/".concat(ID, "/leave")
  }).done(function (data) {});
};
...
open_fixed_section_with_closer_id = function open_fixed_section_with_closer_id(type) {
  var tgl = get_element("#fixed-section-closer--".concat(type));
  if (tgl) {
    target = [tgl.parentNode];
    toggle_target_data(tgl, target);
  }
